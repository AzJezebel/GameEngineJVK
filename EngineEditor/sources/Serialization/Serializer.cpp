...
namespace XML
{
		...
		for (int i = 0; i < actor.m_children.size(); i++)
		{
			...
			if (actor.m_children[i]->m_name == "CustomActor" || actor.m_children[i]->m_bIsOG)
			{
				if (actor.m_children[i]->GetMeshComponent() != nullptr)
				{
					...
					s.clear();
					s = actor.m_children[i]->GetMeshComponent()->m_staticMeshName;
					child->SetAttribute("ObjName", s.c_str());
					s.clear();
					s = actor.m_children[i]->GetMeshComponent()->m_staticMeshTextureName;
				...
				if (physicsComponent != nullptr)
				{
					...
					if (dim.z != (float)INVALID_INDEX)
					{
						...
						child->SetAttribute("xExtent", dim.x);
						child->SetAttribute("yExtent", dim.y);
						child->SetAttribute("zExtent", dim.z);
					}
					else
					{
						if (dim.y != (float)INVALID_INDEX)
						{
							...
							child->SetAttribute("Radius", dim.x);
							child->SetAttribute("HalfHeight", dim.y);
						}
						else
						{
				...
				if (actor.m_children[i]->GetLightComponent<PointLight>() != nullptr && actor.m_children[i]->GetLightComponent<PointLight>()->type == Type::POINT)
				{
					...
					child->SetAttribute("kConstant", actor.m_children[i]->GetLightComponent<PointLight>()->GetConstantAttenuation());
					child->SetAttribute("kLinear", actor.m_children[i]->GetLightComponent<PointLight>()->GetLinearAttenuation());
					child->SetAttribute("kQuad", actor.m_children[i]->GetLightComponent<PointLight>()->GetQuadraticAttenuation());
				}
				...
				if (actor.m_children[i]->GetLightComponent<SpotLight>() != nullptr && actor.m_children[i]->GetLightComponent<SpotLight>()->type == Type::SPOT)
				{
					...
					child->SetAttribute("Direction", ss.str().c_str());
					child->SetAttribute("Cutoff", actor.m_children[i]->GetLightComponent<SpotLight>()->GetCutOff());
					child->SetAttribute("OutCutoff", actor.m_children[i]->GetLightComponent<SpotLight>()->GetOuterCutOff());
					ss.clear();
					...
					child->SetAttribute("Specular", ss.str().c_str());
					child->SetAttribute("kConstant", actor.m_children[i]->GetLightComponent<SpotLight>()->GetConstantAttenuation());
					child->SetAttribute("kLinear", actor.m_children[i]->GetLightComponent<SpotLight>()->GetLinearAttenuation());
					child->SetAttribute("kQuad", actor.m_children[i]->GetLightComponent<SpotLight>()->GetQuadraticAttenuation());
				}
				if (actor.m_children[i]->GetSoundComponent() != nullptr)
				{
					...
					child->SetAttribute("Volume", actor.m_children[i]->GetSoundComponent()->m_volume);
					child->SetAttribute("Frequency", actor.m_children[i]->GetSoundComponent()->m_frequency);
		...
		for (pChild; pChild; pChild = pChild->NextSiblingElement())
		{
			...
			if (m_bCustom)
			{
				if (pChild->Attribute("StaticMesh") != nullptr)
				{
					...
					objPath += pChild->Attribute("ObjName");
					std::string texturePath = "resources\\images\\";
					texturePath += pChild->Attribute("TextureName");
				...
				{
					DirectionalLight dirLight(TextToVec3(pChild->Attribute("Position")),
						TextToVec3(pChild->Attribute("Ambient")),
						TextToVec3(pChild->Attribute("Diffuse")),
						TextToVec3(pChild->Attribute("Specular")));
	...
	Actor* Serializer::GetActorType(const char* text, std::pair<std::string, std::string> paths)
	{
		...
		else if (text == std::string("Player"))
			newActor = new Player{};
		else if (text == std::string("Weapon"))
			newActor = new Weapon{};
		else if (text == std::string("StaticBox"))
			newActor = new StaticBox{};
		else if (text == std::string("Mountain"))
			newActor = new Mountain{};
		else if (text == std::string("Sphere"))
			newActor = new Sphere{};
