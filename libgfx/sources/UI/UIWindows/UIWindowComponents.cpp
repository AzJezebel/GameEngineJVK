...
void UIWindowComponents::DisplayBasicActorSettings()
{
	...
	if (ImGui::InputText("##ActorNameInput", m_actorToDisplay->m_actorNameInput.data(), IM_ARRAYSIZE(m_actorToDisplay->m_actorNameInput.data()), ImGuiInputTextFlags_EnterReturnsTrue))
	{
...
void UIWindowComponents::DisplayPhysixComponent()
{
	if (IUIWidgets::CreateHeaderComponent("Physics"))
	{
		...
		if (GetPhysicComponentData().GetShape() != "Convex")
		{
			...
			{
				if (ImGui::InputFloat("Radius##ScalePhysic", &GetPhysicComponentData().GetScalePhysic().x))
				{
					m_bIsPhysicsUpdated = true;
				}
				if (ImGui::InputFloat("Half Height##ScalePhysic", &GetPhysicComponentData().GetScalePhysic().y))
				{
					m_bIsPhysicsUpdated = true;
				}
			}
...
void UIWindowComponents::DisplayLightComponent()
{
	if (IUIWidgets::CreateHeaderComponent("Light"))
	{
		...
		IUIWidgets::CreateColorButtonAndPickerOnPopup("##AmbiantColor", "MyColor##4", "Popup AmbiantColor", GetLightComponentData().GetAmbiantColorLight(), m_bIsBasicLightSettingsUpdated);

		IUIWidgets::CreateComponentContentTitle("Diffuse Color");
		IUIWidgets::CreateColorButtonAndPickerOnPopup("##DiffuseColor", "MyColor##5", "Popup DiffuseColor", GetLightComponentData().GetDiffuseColorLight(), m_bIsBasicLightSettingsUpdated);

		IUIWidgets::CreateComponentContentTitle("Specular Color");
		IUIWidgets::CreateColorButtonAndPickerOnPopup("##SpecularColor", "MyColor##6", "Popup SpecularColor", GetLightComponentData().GetSpecularColorLight(), m_bIsBasicLightSettingsUpdated);
		...
		IUIWidgets::CreateVec3Content(m_textures[static_cast<int>(TextureNameComponentsWindow::X_AXIS)].GetTextureID(),
			m_textures[static_cast<int>(TextureNameComponentsWindow::Y_AXIS)].GetTextureID(),
			m_textures[static_cast<int>(TextureNameComponentsWindow::Z_AXIS)].GetTextureID(),
			"##PosXLight", "##PosYLight", "##PosZLight", GetLightComponentData().GetLightPosition(), m_bIsBasicLightSettingsUpdated);
